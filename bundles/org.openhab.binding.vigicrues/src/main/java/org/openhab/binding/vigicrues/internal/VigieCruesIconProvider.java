...
/**
 * The {@link VigieCruesIconProvider} is the class providing binding related icons.
 *
 * @author Gaël L'hopital - Initial contribution
 */
@Component(service = { IconProvider.class, VigieCruesIconProvider.class })
@NonNullByDefault
public class VigieCruesIconProvider implements IconProvider {
    private static final String DEFAULT_LABEL = "VigiCrues Icons";
    private static final String DEFAULT_DESCRIPTION = "Icons illustrating water flooding alert levels";
    private static final Set<String> ICONS = Set.of("crue", "vigieau");

    private final Logger logger = LoggerFactory.getLogger(VigieCruesIconProvider.class);
    private final BundleContext context;
    private final TranslationProvider i18nProvider;

    @Activate
    public VigieCruesIconProvider(final BundleContext context, final @Reference TranslationProvider i18nProvider) {
    ...
    public @Nullable InputStream getIcon(String category, String iconSetId, @Nullable String state, Format format) {
        ...
        if (state != null) {
            String iconWithState = getResource("%s-%s".formatted(category, state));
